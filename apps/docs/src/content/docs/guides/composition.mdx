...
| Layer | Package | Role |
| --- | --- | --- |
| Client | `@paykernel/core` | `createPaymentClient` + gateway factories (`moyasarGateway`, `stripeGateway`, `paypalGateway`, `paymobGateway`). Extra gateways: [`@paykernel/gateway-tap`](/gateways/tap), [`@paykernel/gateway-myfatoorah`](/gateways/myfatoorah), [`@paykernel/gateway-hesabe`](/gateways/hesabe) (not `BuiltInGatewayName`). |
