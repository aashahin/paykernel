...
**Merchant-account testing was blocked on 2026-09-11.** Both supplied tokens returned HTTP 401; an official public sandbox credential succeeded in a separate control check. No completed checkout or refund with the supplied account is recorded. See the [gateway validation matrix](/guides/gateway-validation) for scope and evidence.
:::
