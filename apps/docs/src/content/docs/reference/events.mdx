...
Tables are pure data (`STRIPE_EVENT_TYPE_MAP`, `MOYASAR_EVENT_TYPE_MAP`, `PAYPAL_EVENT_TYPE_MAP`, `PAYMOB_TOKEN_EVENT_TYPES`, `STRIPE_UNMAPPED_EVENT_TYPES`) plus `mapProviderEventTypeToStable`. Extra gateways ([Tap](/gateways/tap), [MyFatoorah](/gateways/myfatoorah), [Hesabe](/gateways/hesabe)) are not in these core maps.
