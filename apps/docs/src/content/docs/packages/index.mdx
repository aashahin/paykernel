---
title: Packages
description: Map of every publishable PayKernel package (core 1.0.0, gateways/testkit 1.0.1, others 0.1.0/0.1.1) with npm names and exports.
sidebar:
  order: 1
---
...
Published versions: `core` **1.0.0**, `gateway-tap`/`gateway-myfatoorah` **1.0.1**, `testkit` **1.0.2**, `gateway-hesabe` **0.1.0**, `webhooks`/`reconciliation`/`routing`/`opentelemetry`/`stores`/`integrations` **0.1.1**, `store-d1` **0.1.2**, `store-contracts`/`sql-foundation` **0.1.0**. All packages below are **published** to the public registry. Install with `bun add @paykernel/<name>`.
